...
class PhrasesView extends View {
    ...
    public function show($phrases, $paginaActual, $totalPaginas, $busqueda) {
        ...
        foreach ($phrases as $phrase) {
            echo "<tr>
            <td>{$phrase->__get('texto')}</td>
             <td><a href=\"?Author/showAuthorDetalle&id={$phrase->__get('id_autor')}\">{$phrase->__get('autor')}</a></td>
            <td>{$phrase->__get('temas')}</td>
            <td class=\"casillaBotonesFrases\">
                <a href=\"?Phrases/showEdit&id={$phrase->__get('id_frase')}\">
                    <button class=\"botonEditar\">Editar</button>
                </a>
                <a href=\"?Phrases/delete&id={$phrase->__get('id_frase')}\"
        ...
        echo "<main>
                ...
                <div class=\"formEditar\">
                    <form action=\"?Phrases/form\" method=\"post\">
                        <input type=\"hidden\" name=\"id\" value=\"{$phrase->__get('id_frase')}\">
                        
                        <div class=\"campoEditar\">
                            <label for=\"fraseEditar\">Frase (*)</label>
                            <textarea id=\"fraseEditar\" name=\"texto\" required>{$phrase->__get('texto')}</textarea>
                        </div>
                        ...
                        foreach ($autores as $autor) {
                            $selected = ($autor->__get('id_autor') == $phrase->__get('id_autor')) ? "selected" : "";
                            echo "<option value=\"{$autor->__get('id_autor')}\" $selected>{$autor->__get('nombre')}</option>";
                        }
                        ...
                        foreach ($temasDisponibles as $tema) {
                            $selected = ($tema->__get('id_tema') == $phrase->__get('temas')) ? "selected" : ""; 
                            echo "<option value=\"{$tema->__get('id_tema')}\" $selected>{$tema->__get('nom')}</option>";
                        }
                ...
                <form action=\"?Phrases/form\" method=\"post\">
                    ...
                    <div class=\"campoEditar\">
                        ...
                        foreach ($authors as $author) {
                            $selected = ($author->__get('id_autor') == $phrase->__get('id_autor')) ? "selected" : "";
                            echo "<option value=\"{$author->__get('id_autor')}\" $selected>{$author->__get('nombre')}</option>";
                        }
                        echo "</select>
                        $errorAutor
                    </div>
                    ...
                    <div class=\"campoEditar\">
                        ...
                        foreach ($themes as $theme) {
                            $selected = ($theme->__get('id_tema') == $phrase->__get('temas')) ? "selected" : "";
                            echo "<option value=\"{$theme->__get('id_tema')}\" $selected>{$theme->__get('nom')}</option>";
                        }
                        echo "</select>
                        $errorTema
                    </div>
    ...
    public function showCrearFrases($autores, $temasDisponibles, $id_tema=null, $id_autor=null) {
        ...
        foreach ($autores as $autor) {
            $selected = ($id_autor == $autor->__get('id_autor')) ? 'selected' : '';
            echo "<option value=\"{$autor->__get('id_autor')}\" $selected>{$autor->__get('nombre')}</option>";
        }
        ...
        foreach ($temasDisponibles as $tema) {
            $selected = ($id_tema == $tema->__get('id_tema')) ? 'selected' : '';
            echo "<option value=\"{$tema->__get('id_tema')}\" $selected>{$tema->__get('nom')}</option>";
        }
        ...
        echo "<main>
                ...
                <div class=\"formEditar\">
                    <form action=\"?Phrases/form\" method=\"post\">
                        ...
                        <div class=\"campoEditar\">";
                    $textareaValue = $phrase ? $phrase->__get("texto") : "";
                    ...
                    <div class=\"campoEditar\">";
                        ...
                        foreach ($autores as $autor) {
                            $selected = ($phrase && $phrase->__get('id_autor') == $autor->__get('id_autor')) ? 'selected' : '';
                            echo "<option value=\"{$autor->__get('id_autor')}\" $selected>{$autor->__get('nombre')}</option>";
                        }
                    ...
                    <div class=\"campoEditar\">";
                        ...
                        foreach ($temasDisponibles as $tema) {
                            $selected = ($phrase && $phrase->__get('id_tema') == $tema->__get('id_tema')) ? 'selected' : '';
                            echo "<option value=\"{$tema->__get('id_tema')}\" $selected>{$tema->__get('nom')}</option>";
                        }
