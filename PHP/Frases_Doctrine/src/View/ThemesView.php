...
class ThemesView extends View {
    ...
    public function show($themes, $paginaActual, $totalPaginas, $busqueda) {
        ...
        foreach ($themes as $theme) {
            echo "<tr>
                <td><a href=\"?Themes/showThemeDetalle&id={$theme->getId()}\">{$theme->getName()}</a></td>
                <td>{$theme->getNumFrases()}</td>
                <td class=\"casillaBotonesFrases\">
                    <a href=\"?Themes/showEditar&id={$theme->getId()}\"><button class=\"botonEditar\">Editar</button></a>
                    <a href=\"?Themes/delete&id={$theme->getId()}\" onclick=\"return confirm('¿Seguro que deseas eliminar este tema?');\"><button class=\"botonEliminar\">Eliminar</button></a>
                    <a href=\"?Phrases/showCrear&id={$theme->getId()}\"><button class=\"botonCrear\">Crear</button></a>
                </td>
            </tr>";
        }
            ...
            <div class=\"formEditar\">
                <form action=\"?Themes/form\" method=\"post\">
                    <div class=\"campoCrear\">
                        ...
                        <input type=\"text\" id=\"temaCrear\" name=\"temaEditar\" value=\"{$tema->getName()}\" required>
                        $errorTema
                    </div>
                    $errorGeneral
    ...
    public function showEditarTema(Theme $theme){
        ...
        echo '<main>
            ...
            <div class="formEditar">
                <form action="?Themes/form" method="post">
                    <input type="hidden" name="id_tema" value="' . $theme->getId() . '">
                    <div class="campoEditar">
                        <label for="temaEditar">Tema (*)</label>
                        <input type="text" id="temaEditar" name="temaEditar" value="' . $theme->getName() . '" required>
                    </div>
    ...
    public function showDetail($theme, $phrases) {
        ...
        include "inc/menuMain.php";
        echo "<div class=\"frases-content\">
        <h1>Frases sobre \"{$theme->getName()}\"</h1>
        ...
        if (empty($phrases)) {
            ...
            foreach ($phrases as $phrase) {
                ...
                $autor = $phrase->getAuthor() ?: "Autor desconocido";
                $authorName = $autor->getName();
                echo "<tr>
                <td>{$texto}</td>
                <td>{$authorName}</td>
            </tr>";
