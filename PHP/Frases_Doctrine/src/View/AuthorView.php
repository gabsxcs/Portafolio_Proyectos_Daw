...
class AuthorView extends View {
    ...
    public function show($authors, $paginaActual, $totalPaginas, $busqueda) {
        ...
        foreach ($authors as $author) {
            echo "<tr>
                <td><a href=\"?Author/showAuthorDetalle&id={$author->getId()}\">{$author->getName()}</a></td>
                <td>{$author->getDescription()}</td>
                <td>{$author->getNumFrases()}</td>
                <td class=\"casillaBotonesFrases\">
                    <a href=\"?Author/showEditar&id={$author->getId()}\">
                        <button class=\"botonEditar\">Editar</button>
                    </a>
                    <a href=\"?Author/delete&id={$author->getId()}\"
                       onclick=\"return confirm('¿Seguro que deseas eliminar este autor?');\">
                        <button class=\"botonEliminar\">Eliminar</button>
                    </a>
                    <a href=\"?Phrases/showCrear&idAutor={$author->getId()}\">
    ...
    public function showEditarAutor(Author $author) {
        ...
        <div class="formEditar">
            <form action="?Author/form" method="post">
                <input type="hidden" name="id_autor" value="' . $author->getId() . '">  <!-- Aquí se usa el getter -->
                    
                <div class="campoEditar">
                    <label for="nombre">Autor (*)</label>
                    <input type="text" id="nombre" name="nombre" value="' . $author->getName() . '" required> <!-- Aquí se usa el getter -->
                </div>
                        
                <div class="campoEditar">
                    <label for="descripcion">Descripción (*)</label>
                    <textarea id="descripcion" name="descripcion" required>' . $author->getDescription() . '</textarea>  <!-- Aquí se usa el getter -->
                </div>
    ...
    public function formEditar(Author $author, $errores = []) {
        ...
        echo '<main>
            ...
            <div class="formEditar">
                <form action="?Author/form" method="post">
                    <input type="hidden" name="id_autor" value="'. $author->getId() .'">
                        
                    <div class="campoEditar">
                        <label for="autorEditar">Autor (*)</label>
                        <input type="text" id="autorEditar" name="nombre" value="'. $author->getName() .'" required>
                        ' . $errorNombre . '
                    </div>
                            
                    <div class="campoEditar">
                        <label for="descripcionEditar">Descripción (*)</label>
                        <textarea id="descripcionEditar" name="descripcion" required>'. $author->getDescription() .'</textarea>
                        ' . $errorDescripcion . '
                    </div>' .
                         $errorGeneral   . '
                    <button id="btnEditarFrase" type="submit">Enviar</button>
                    ...
                    <form action=\"?Author/form\" method=\"post\">
                        <div class=\"campoCrear\">
                            ...
                            <input type=\"text\" id=\"autorCrear\" name=\"nombre\"
                                   value=\"" . htmlspecialchars($author->getName()) . "\" required>
                            $errorNombre
                        </div>
                        ...
                        <div class=\"campoCrear\">
                            ...
                            . htmlspecialchars($author->getDescription()) . "</textarea>
                            $errorDescripcion
                        </div>
                        $errorGeneral
                        <button id=\"btnCrearAutor\" type=\"submit\">Crear Autor</button>
    ...
    public function showDetail($author, $phrases) {
        ...
        include "inc/menuMain.php";
        echo "<div class=\"frases-content\">
    <h1>Frases de {$author->getName()}</h1>
    <p>{$author->getDescription()}</p>
        ...
        <p>No hay frases registradas para este autor.</p>
            ...
            foreach ($phrases as $phrase) {
                $texto = $phrase->getTexto();
                
                $temas = $phrase->getThemes();
                $temas = $temas->isEmpty() ? "Sin tema" : implode(', ', array_map(function($theme) {
